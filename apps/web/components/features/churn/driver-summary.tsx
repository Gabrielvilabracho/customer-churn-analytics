...
export function DriverSummary({ model }: DriverSummaryProps) {
  const leadingCohort = model.majorCategoryCohorts[0];
      ...
      <CardContent>
        {leadingCohort && topRisk ? (
          <p className="text-sm leading-6 text-muted-foreground">
            {leadingCohort.majorCategory} students dominate the sampled risk pool, with {leadingCohort.averageChurnProbability} average burnout risk. Prioritize outreach for {topRisk.displayReference}, currently labeled {topRisk.riskLabel.toLowerCase()}.
          </p>
        ) : (
