...
class TestLeakageExclusion:
    ...
    def test_from_rows_merges_additional_exclusions_with_defaults(self) -> None:
        """Caller must NOT be able to reintroduce leakage columns by passing a
        partial excluded_feature_columns. Providing only an additional exclusion
        must still exclude both default leakage columns AND the additional field."""
        csv_path = FIXTURE_DIR / "education_sample.csv"
        rows = list(csv.DictReader(csv_path.open(newline="", encoding="utf-8")))

        # Only pass the additional exclusion — NOT the leakage columns
        dictionary = FeatureDictionary.from_rows(
            rows,
            customer_key="Student_ID",
            target_column="Burnout_Risk_Level",
            excluded_feature_columns=frozenset({"Anxiety_Level_During_Exams"}),
        )

        feature_names = {f.name for f in dictionary.features}
        # Leakage columns MUST still be excluded (merged from defaults)
        assert "Post_Semester_GPA" not in feature_names, (
            "Post_Semester_GPA must be excluded even when caller only passes additional exclusions"
        )
        assert "Skill_Retention_Score" not in feature_names, (
            "Skill_Retention_Score must be excluded "
            "even when caller only passes additional exclusions"
        )
        # Additional exclusion must also apply
        assert "Anxiety_Level_During_Exams" not in feature_names, (
            "Anxiety_Level_During_Exams must be excluded when passed by caller"
        )
        # 16 cols - Student_ID - Burnout_Risk_Level - 2 leakage - 1 extra = 11
        assert len(feature_names) == 11, (
            f"Expected 11 features (16 cols - 2 keys - 2 leakage - 1 extra); "
            f"got {len(feature_names)}: {sorted(feature_names)}"
        )
